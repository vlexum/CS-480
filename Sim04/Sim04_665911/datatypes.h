...
typedef struct InterruptData
   {
    double blockedEndTime;             // End time for when the operation is
                                       // finished and ready to interrupt
    bool serviced;

    ProcessType *process;       // pointer to start of process in op codes
    struct InterruptData *nextInterrupt;  // pointer to next process
} InterruptData;
