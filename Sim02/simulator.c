    ...
    while ( metaDataMstrPtr != NULL )
       {
        ...
        // do not need to do math with mem op
        else if ( compareString( metaDataMstrPtr->command, "mem" ) != STR_EQ )
           {
            ...
            // check if NOT sys op
            else if ( compareString( metaDataMstrPtr->command, "sys" ) !=       STR_EQ )
               {
...
void runSim( ConfigDataType *configPtr, OpCodeType *metaDataMstrPtr )
   {
    ...
    // int memAvailable = configPtr->memAvailable;
    ...
    // diplay process change
       // function: manageDisplay
    manageDisplay( currentProcess, PROCESS_CHANGE, NULL );
    ...
    while ( processCounter > 0 && pcbPtr != NULL && runFlag )
       {
            ...
            if ( compareString( opCode->strArg1, "allocate" ) == STR_EQ )
               {
                // diplay operation info
                  // function: manageDisplay
                manageDisplay( currentProcess, PRINT_MESSAGE, "Memory allocate stuff called here\n" );
               }
            ...
            else if ( compareString( opCode->strArg1, "access" ) == STR_EQ )
               {
                // diplay operation info
                   // function: manageDisplay
                manageDisplay( currentProcess, PRINT_MESSAGE, "Memory access stuff called here\n" );
               }
           }
        ...
        if ( compareString( opCode->command, "app" ) == STR_EQ &&
             compareString( opCode->strArg1, "end" ) == STR_EQ )
           {
            ...
            // decrement processCounter because process is complete
            processCounter--;
           }
